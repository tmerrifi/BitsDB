...
void _fork_children(int size_of_segment, char * segment_name, 
                    ksnap_test_subs_profile * test_profile, void (*action) (ksnap * snap, ksnap_test_subs_profile * test_profile));
...
void ksnap_test_subs_init(int num_of_subscribers, int type_of_subscribers, int size_of_segment, 
  ...
  for(int i=0; i < num_of_subscribers; ++i){
    ...
    if (test_profile && test_profile->action){
      _fork_children(size_of_segment, segment_name, test_profile, test_profile->action);
    }
    else{
	...
	_fork_children(size_of_segment, segment_name, test_profile, _main_snap);
	break;
      }
    }  
  }
